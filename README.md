...
- Web 页面会实时显示传输进度条、字节数和分片数；客户端与服务端终端也会输出进度，通常每增长 5% 更新一次，低速传输至少每 2 秒更新。
- 每个 UDP 数据报不超过 1232 字节；加密后的文件数据块为 1120 字节，避免常见网络上的 IP 分片。
- 客户端使用最多 64 个分片的滑动窗口并行请求，允许分片乱序到达；丢包时只重试窗口中缺失的分片，不再为每个 1120 字节分片单独等待一次网络往返。
- `-timeout` 是无有效数据进展的超时，只要持续收到握手、元数据或文件分片就会不断重置，不限制传输总时长。
- 完成后校验整个压缩归档的 SHA-256，损坏的传输不会被解包。
- Web 助手只有在归档完整写入浏览器连接后才显示完成；本地响应被截断时会把任务标记为失败。
...
- 每个方向使用经过认证的递增序列号派生 AES-256-GCM nonce；允许认证包在 256 包窗口内乱序到达，重复包和窗口外的过旧包会被拒绝。被篡改的数据包会被丢弃并由重试机制以新序列号恢复。
...
udpfile client（udpfile web 是兼容别名）:
  ...
  -timeout       无有效数据进展超时，默认 10m；持续传输不限制总时长
